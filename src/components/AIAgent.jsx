...
export default function AIAgent() {
  const [messages, setMessages] = useState([
    {
      role: "system",
      text: "أهلاً بك أيها البطل. أنا مساعدك الذكي. كيف يمكنني مساعدتك اليوم؟ (مثال: أضف 500 سعرة، أو سجّل عادة المشي ليوم الاثنين)",
    },
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let currentHabits = [
        "Gym", "Diet", "Study", "Sleep", "Water", "Tidy", "Phone", "Spend",
      ];
      try {
        ...
        if (setRes.ok) {
          ...
          if (sData['tracker:habits']) {
            const parsed = JSON.parse(sData['tracker:habits']);
            currentHabits = parsed.map(h => h.name);
          }
        }
      } catch (e) {}
      ...
      if (data.error) {
        setMessages((p) => [
          ...p,
          { role: "system", text: `⚠️ خطأ: ${data.error}` },
        ]);
        setIsLoading(false);
        return;
      }
      ...
      if (data.actions && data.actions.length > 0) {
        ...
        setMessages((p) => [
          ...
          {
            role: "system",
            text: "لم أتمكن من استخراج إجراء محدد من طلبك. هل يمكنك التوضيح أكثر؟",
          },
        ]);
      }
    } catch (error) {
      ...
      setMessages((p) => [
        ...
        {
          role: "system",
          text: "⚠️ عذراً، لا يمكنني الاتصال بمحرك الذكاء الاصطناعي الآن (تأكد من إعدادات الشبكة و Railway).",
        },
      ]);
    }
    setIsLoading(false);
          ...
          >
            <div
              style={{
                ...
                borderRadius: 12,
                background:
                  msg.role === "user" ? "#3b82f6" : "rgba(99, 102, 241, 0.2)",
                display: "flex",
                alignItems: "center",
